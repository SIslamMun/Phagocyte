...
```bash
# Start the server (from src/ingestor)
uv run ingestor-mcp
```
...
```json
{
  "mcpServers": {
    "ingestor": {
      ...
      "cwd": "/path/to/Phagocyte/src/ingestor"
    }
  }
}
```
...
claude mcp add ingestor -- uv --directory /path/to/Phagocyte/src/ingestor run ingestor-mcp

# Or if running from ingestor directory
cd /path/to/Phagocyte/src/ingestor
...
```json
{
  "mcp.servers": {
    "ingestor": {
      "command": "uv",
      "args": ["run", "ingestor-mcp"],
      "cwd": "${workspaceFolder}/src/ingestor"
    }
  }
}
```
