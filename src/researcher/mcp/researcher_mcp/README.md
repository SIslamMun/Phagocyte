...
```bash
# From the Phagocyte root
cd Phagocyte/src/researcher
...
```bash
# Start the server (from src/researcher)
...
```json
{
  "mcpServers": {
    "researcher": {
      ...
      "cwd": "/path/to/Phagocyte/src/researcher",
      "env": {
...
claude mcp add researcher -- uv --directory /path/to/Phagocyte/src/researcher run researcher-mcp

# Or if running from researcher directory
cd /path/to/Phagocyte/src/researcher
...
```json
{
  "mcp.servers": {
    "researcher": {
      "command": "uv",
      "args": ["run", "researcher-mcp"],
      "cwd": "${workspaceFolder}/src/researcher",
