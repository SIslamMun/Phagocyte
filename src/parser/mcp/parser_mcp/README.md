...
```bash
# Start the server (from src/parser)
uv run parser-mcp
```
...
```json
{
  "mcpServers": {
    "parser": {
      "command": "uv",
      "args": ["run", "parser-mcp"],
      "cwd": "/path/to/Phagocyte/src/parser",
      "env": {
...
claude mcp add parser -- uv --directory /path/to/Phagocyte/src/parser run parser-mcp

# Or if running from parser directory
cd /path/to/Phagocyte/src/parser
...
```json
{
  "mcp.servers": {
    "parser": {
      "command": "uv",
      "args": ["run", "parser-mcp"],
      "cwd": "${workspaceFolder}/src/parser",
