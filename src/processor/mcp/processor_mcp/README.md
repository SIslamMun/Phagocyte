...
```bash
# Start the server (from src/processor)
...
```json
{
  "mcpServers": {
    "processor": {
      ...
      "cwd": "/path/to/Phagocyte/src/processor"
    }
  }
}
```
...
claude mcp add processor -- uv --directory /path/to/Phagocyte/src/processor run processor-mcp

# Or if running from processor directory
cd /path/to/Phagocyte/src/processor
...
```json
{
  "mcp.servers": {
    "processor": {
      "command": "uv",
      "args": ["run", "processor-mcp"],
      "cwd": "${workspaceFolder}/src/processor"
    }
  }
}
```
