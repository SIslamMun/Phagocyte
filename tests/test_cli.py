...
class TestCommandCounts:
    ...
    def test_parse_command_count(self, runner: CliRunner) -> None:
        ...
        expected = ["refs", "retrieve", "batch", "doi2bib", "verify", 
                   "citations", "sources", "auth", "init", "config"]
        for cmd in expected:
    ...
    def test_process_command_count(self, runner: CliRunner) -> None:
        ...
        expected = ["run", "search", "stats", "setup", "check", 
                   "export", "import", "visualize", "deploy", "server", "test-e2e"]
        for cmd in expected:
            assert cmd in result.output, f"Missing command: {cmd}"
